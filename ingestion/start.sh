...
then
    ...
    echo "$SPARK_SERVICE is not running"
        nohup /opt/spark/spark-2.4.4-bin-hadoop2.7/bin/spark-submit --class FromSocketToKafka socketToKafka/target/billiardstream-1.0-SNAPSHOT-jar-with-dependencies.jar $TOPIC $STREAM_HOST $STREAM_PORT > $LOGS_DIRECTORY/stream_$DATE.log 2>&1 &
fi
